...
#define vector_push_back(T, vec, elem) /* -> vector_ptr_t */              \
  ({                                                                      \
    ...
    vector_ptr_t grown__ = (vector_size(vec__) == vector_capacity(vec__)) \
                               ...
                               : vec__;                                   \
    if (grown__ != NULL) {                                                \
      vector_data(T, grown__)[(grown__->size)++] = elem__;                \
    }                                                                     \
    grown__;                                                              \
  })
...
#define vector_pop_back(T, vec) /* -> T */                   \
  ({                                                         \
    m_assert_istype(T);                                      \
    vector_ptr_t vec__ = vec;                                \
    T res__ = vector_data(T, vec__)[vector_size(vec__) - 1]; \
...
#define vector_end(T, vec) /* -> const T* */ \
  ({                                         \
    m_assert_istype(T);                      \
    vector_ptr_t vec__ = vec;                \
    const T* data__ = vector_data(T, vec__); \
...
#define vector_foreach(T, vec, op) /* -> void */             \
  {                                                          \
    ...
    const T* end__ = vector_end(T, vec__);                   \
